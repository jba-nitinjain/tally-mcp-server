...
import { ledgerNatureOf } from './ledgernature.mjs';
import { displayDate } from './period.mjs';
const round2 = (n) => Math.round(n * 100) / 100;
const isSynthetic = (r, type) => r && !r.guid && r.voucher_type === type;
const isVoucher = (r) => !isSynthetic(r, 'Opening') && !isSynthetic(r, 'Closing');
const spanOf = (c) => `${displayDate(c.fromDate)} to ${displayDate(c.toDate)}`;
/** nature of the ledger, read from the first month whose Closing row resolves it */
export function natureOfChunks(lstDone) {
...
const unreconciledNote = (lst) => lst.length > 0 ? `Month(s) ${lst.join('; ')} do not reconcile on their own (opening + vouchers differs from closing)` : '';
/** Balance Sheet ledger, or one whose nature could not be read: each month must chain to the next */
export function joinChained(lstDone, nature) {
    ...
    const summary = { ...statement.summary, chunked: true, chunkCount: chunks.length, chunkContinuity: lstBreak.length === 0, ledgerNature: nature ?? 'unknown', chunks };
...
export function joinRunningSum(lstDone, whole) {
    const opening = lstDone[0].rows.find(r => isSynthetic(r, 'Opening'));
    ...
    if (lstNote.length > 0)
        summary.note = lstNote.join('. ');
    else
        delete summary.note;
    return { rows: statement.rows, summary };
}
//# sourceMappingURL=ledgerchain.mjs.map