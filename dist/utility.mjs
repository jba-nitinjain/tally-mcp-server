    ...
    String = {
        ...
        decodeEntities(value) {
            if (typeof value !== 'string' || value.indexOf('&') === -1)
                return value;
            return value.replace(/&(?:#(\d{1,7})|#[xX]([0-9a-fA-F]{1,6})|(amp|lt|gt|quot|apos));/g, (match, dec, hex, named) => {
                if (named)
        ...
        format(value, mask) {
            function formatNumber(mask, value) {
                ...
                if (mskFraction.length)
                    retval += '.' + numFraction.toFixed(mskFraction.length).slice(2);
                return retval;
            }
            ;
    ...
    Date = {
        ...
        format(value, mask) {
            ...
            if (/yyyy/.test(mask))
                ...
                mask = mask.replace('yy', vYear.toString().slice(-2));
            else
                ;
            //Month replacement
            if (/MMMM/.test(mask))
