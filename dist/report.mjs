 ...
 */
export async function fetchReport(targetReport, inputParams, options) {
    let retval = { data: undefined };
    const period = inputParams.has('fromDate') || inputParams.has('toDate')
...
async function extractReport(reportConfig, reportInputParams, options) {
    ...
    let lstRow = resultObj && resultObj['DATA'] && Array.isArray(resultObj['DATA']['ROW']) ? resultObj['DATA']['ROW'] : []; //a whole DATA envelope without ROW is a genuine "no rows"
