...
const describeStatement = (type: string): string => type.toUpperCase();
...
function walkNodes(node: any, visit: (n: any) => void, seen: Set<any> = new Set()): void {
 ...
 */
function assertReadOnlyByKeywords(sql: string, parseReason: string): void {
...
function stripLiteralsAndComments(sql: string): string {
    ...
    let i = 0;
    const n = sql.length;
    const blank = (from: number, to: number) => { out += ' '.repeat(to - from); };
    ...
    while (i < n) {
        ...
        if (ch === "'" || ((ch === 'E' || ch === 'e') && next === "'")) {
            ...
            while (j < n) {
                if (escapeString && sql[j] === '\\') { j += 2; continue; }
                if (sql[j] === "'") {
