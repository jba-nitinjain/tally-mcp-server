...
export async function fetchLedgerAccount(inputParams: Map<string, any>, fetcher: ReportFetcher = fetchReport): Promise<LedgerAccountResult> {
    ...
    const lstDone: { range: PeriodRange, rows: any[], summary: Record<string, any> }[] = [];
...
function joinChunks(lstDone: { range: PeriodRange, rows: any[], summary: Record<string, any> }[]): LedgerAccountResult {
    ...
    const summary: Record<string, any> = { ...statement.summary, chunked: true, chunkCount: chunks.length, chunkContinuity: lstBreak.length === 0, chunks };
