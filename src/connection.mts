...
// ---------------------------------------------------------------------------
// Tally host / port used by every request, and its persistence (feedback #56)
//
// Up to v7.8.1 the port chosen by set-tally-connection lived only in a module variable of this
// process. Claude Desktop does not keep one server process for the life of a conversation: it
// relaunches the extension on start-up (twice, after its "era probe"), on every extension reload,
// and whenever the process dies. A ledger-account timeout used to kill the process through an
// unhandled promise rejection, so the very next call ran in a fresh process on the default port
// ("Unable to connect ... port 9000", connectionSource "default"). The choice is therefore kept in
// a small state file as well, which every process of this installation reads, so it survives a
// relaunch until it is changed, reset, or left unused for TALLY_CONNECTION_TTL_HOURS (default 12)
// ---------------------------------------------------------------------------
...
export function isValidPort(port: any): port is number {
...
export function connectionStateFile(): string {
...
let memoryConnection: TallyConnection | null = null;
...
function readStateFile(): StateFileContent | null {
    ...
    try {
        if (!fs.existsSync(file))
            return null;
        let content = JSON.parse(fs.readFileSync(file, 'utf8')) as StateFileContent;
...
function writeStateFile(content: StateFileContent): boolean {
    let file = connectionStateFile();
    try {
...
export function isConnectionPersisted(): boolean {
