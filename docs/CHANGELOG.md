...
Fixed:
* **ledger-account over more than three months on an expense or income ledger came back `reconciled: false` with the wrong closing balance.** For a ledger under a Profit & Loss primary group (Sales Accounts, Purchase Accounts, Direct / Indirect Incomes, Direct / Indirect Expenses and their sub-groups) Tally reports an opening balance of 0 for any period starting after the financial year start. v7.9.0 chained the months (each month's closing must equal the next month's opening), so a month opening at 0 after a month which moved was read as a break: on 01/04 to 30/11 with a debit of 11,64,999 in October only, November opened at 0, the statement closed on November's 0, and the answer was `reconciled: false`, `unexplainedMovement` 1164999, `chunkContinuity` false and the note *the months do not chain*. The ledger's nature is now read from Tally (`$IsRevenue`, the same flag as `bs_pl` on chart-of-accounts, falling back to the primary group name). A Profit & Loss ledger is joined as a running sum: opening of the first month plus every voucher of every month, `chunkContinuity` = `not_applicable_nominal`, and checked once against the closing Tally itself reports for the whole period (the figure trial-balance gives), fetched by a small request carrying only the Opening and Closing lines, inside the same 45s budget. `reconciled` is true when every month reconciles on its own and the running sum equals that whole-period closing within 0.01. The response adds `ledgerNature`, `runningClosingBalance`, `wholePeriodClosingBalance` and `wholePeriodCheck` (matched / mismatch / not_done). When the whole-period request does not fit in what is left of the budget, the statement is still returned, `reconciled` resting on the months alone, with a note that the cross-check was not done. A Balance Sheet ledger is chained exactly as before. When the nature cannot be read at all, the months are chained as before, but when every month reconciles and the only breaks are *opens at 0 after a non-zero close*, the note says so plainly instead of calling the statement incomplete (feedback #59)
* Tests: a mocked Tally reproduces the reported case (expense ledger, one month of eight moving) and the same range on a Balance Sheet ledger, plus the whole-period check timing out, disagreeing, and an unreadable nature

### Version: v7.9.0 [24/09/2026]

Fixed:
* **A full-year ledger-account on a busy ledger answered rowCount 0, *Tally returned no rows*, although the two half-years each returned rows and reconciled.** The report took longer than the 45s budget. The timeout was raised inside a promise executor whose `finally` resolved the result first, so the caller received an empty result and the error escaped as an unhandled promise rejection, which **ended the server process**. Report failures are now returned, not thrown past the result: a timeout, a socket reset, an answer aborted part way, an empty body and a `<DATA>` envelope which never closes all come back as an error object `{ code: "TALLY_TIMEOUT", reason, phase, elapsedMs, period, hint: "split the period", message }`. *No rows* is reported only when Tally delivered a whole `<DATA>` envelope holding no row. Every other report and collection tool (trial-balance, profit-loss, balance-sheet, stock-summary, bills-outstanding, list-master, query-collection and the rest) answers a transport failure with the same object, carrying the period of the call. The message no longer promises that an identical retry will succeed when Tally was still computing: it says to split the period, and that Tally may stay busy for up to a minute finishing the abandoned request (feedback #56)
* **set-tally-connection to a non-default port was followed by *Unable to connect ... port 9000*, and server-info showed connectionSource `default`.** The port lived only in the memory of the server process, and the process did not live as long as the conversation: the crash above ended it, and Claude Desktop also relaunches extensions on start-up, on reload and whenever one stops. The choice is now also kept in a small state file (`%USERPROFILE%\.tally-mcp-server\connection-<default host>_<default port>.json`, folder overridable with `TALLY_STATE_DIR`) which every process of the installation reads, so it survives a relaunch; server-info reports connectionSource `session` until it is changed, and lapses to the installed default after 12 hours without use (`TALLY_CONNECTION_TTL_HOURS`). server-info also reports `connectionPersisted`, `connectionStateFile`, `processId` and `processUptimeSeconds`, so a relaunch is visible. set-tally-connection now probes the new port before switching, so a mistyped port never replaces the connection in use, not even for a moment. An unhandled promise rejection is now logged instead of ending the server (feedback #56)
* A ledger or stock item name containing a double quote broke the TDL string literal of ledger-account and stock-item-account; it is now doubled as TDL expects. Names containing an apostrophe were already sent correctly XML-escaped (`&#39;`), which a regression test now asserts along with the double quote and the ampersand

Changed:
* **ledger-account fetches a period longer than three months one calendar month at a time** and joins the months into one statement: the first month's Opening row, every voucher, the last month's Closing row. The months share one budget (`TALLY_TIMEOUT_MS`, 45s) so the tool still answers before the Claude relay gives up at 60s. Each month must reconcile on its own (opening + vouchers = closing) and chain to the next (its closing equals the next month's opening); the response carries `chunked`, `chunkCount`, `chunkContinuity` and `chunks` (per-month balances and voucher count), and `reconciled` is false with a note naming the months when either check fails. When the months do not all fit in the budget, no partial statement is returned: the answer is TALLY_TIMEOUT with `monthsRetrieved`, `failedPeriod` and `suggestedPeriods` (the months already fetched as one period, the rest in quarters) to call one by one
* Tests: `npm test` builds and runs a regression suite (node:test) against a mocked Tally which delays past the timeout, resets, aborts, truncates or answers empty, plus month chunking, name escaping and connection persistence across server processes
